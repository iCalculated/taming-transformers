...
class Net2NetTransformer(pl.LightningModule):
    def __init__(
        ...
        self.init_cond_stage2_from_ckpt(cond_stage_config2)
    ...
    def init_cond_stage2_from_ckpt(self, config):
        if config != None:
            model = instantiate_from_config(config)
            model = model.eval()
            model.train = disabled_train
            self.cond_stage2_model = model

    def forward(self, x, c):
        # one step to produce the logits
        # TODO: how should c be encoded?

        _, z_indices = self.encode_to_z(x)
        _, c_indices = self.encode_to_c(c)
        _, c2_indices = self.encode_to_c2(c)

        c_indices = torch.cat((c_indices, c2_indices), dim=1)
    ...
    def encode_to_c2(self, c):
        ...
        quant_c, _, [_, _, indices] = self.cond_stage_model2.encode(c)
        if len(indices.shape) > 2:
            indices = indices.view(c.shape[0], -1)
        return quant_c, indices
    ...
    def log_images(
        ...
        **kwargs,
        ...
        # this is probably good future sasha
        quant_z, z_indices = self.encode_to_z(x)
        quant_c, c_indices = self.encode_to_c(c)  # TODO: change to c1 somehow
        quant_c2, c_indices2 = self.encode_to_c2(c)  # TODO: change to c2 somehow

        quant_c = torch.cat((quant_c, quant_c2), dim=1)
        c_indices = torch.cat((c_indices, c_indices2), dim=1)
